...
vector_Headings = 0:45:360;   %deg
timeSim = "300";   %s
averageFomTime = 0; %s

for idx=1:length(vector_Headings)
    vector_Headings_rad = vector_Headings(idx)/180*pi;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            Significant_wave_height_Hs = 5;  %m
            Tp = 10;                         % sec
            Mean_wave_direction = vector_Headings_rad;     %rad  % Wave Angle defined as where the wind is coming from [rad]
            ...
            % reference sstem
            CurrHead.HeadStart = vector_Headings_rad;     %[rad] initial heading value (alway to be defined, it 
                                            % is used also for he case of "constant
                                            % heading"
            CurrHead.HeadEnd = vector_Headings_rad;       %[rad] final heading value
            ...
            Wind_U10 = 15;                  % Wind speed at 10 m levation 
            Wind_Head_mean = vector_Headings_rad;         % Wind Angle defined as where the wind is coming from [rad]
            ...
            %% SET-POINTS INPUT
            typeOfSimulation = 0; % 0=constant set-point , 1 sequence of set-points, 2 OPTSIM
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    time_index = find(u_output.Time > averageFomTime,1,'First');
    Time = u_output.Time(time_index:end);
    uAbs = abs(u_output.Data);
    u_tot=sum(uAbs(:,1:end)');
    u_tot_averaqe = mean(u_tot(time_index:end));

   
    figure()
    plot(u_output.Time,u_tot);
    hold on
    plot(u_output.Time(time_index:end),u_tot(time_index:end));
    plot([Time(1) Time(end)],[u_tot_averaqe u_tot_averaqe]);
    hold off
    
    Total_force(idx) =  u_tot_averaqe;
    Heading(idx) = vector_Headings(idx);
    
    clear Time time_index u_tot u_tot_averaqe
    

end

figure()
polarplot(Heading*pi/180,Total_force,'-o');
hold on
polarplot(Heading*pi/180,ones(length(Heading),1)*Thuster_tot_thrust,'-','Linewidth',2);
set(gca,'ThetaZeroLocation','top')
title('Thrust used [kN]')
legend('Used','Limit');


figure()
polarplot(Heading*pi/180,Total_force/Thuster_tot_thrust*100,'-o');
hold on
polarplot(Heading*pi/180,ones(length(Heading),1)*100,'-','Linewidth',2);
set(gca,'ThetaZeroLocation','top')
title('Thrust usage [%]')
legend('Used','Limit');
